...
// ── Data akurasi per metode (Dzulvikar 2025) ─────────────────────────────
const METHODS = {
    sfmmvs: {
        label: 'SfM-MVS',
        desc: 'Dense Mesh (OBJ/GLTF)',
        color: 0x2DD4BF,
        ce90: '2.74', le90: '11.46', rmseH: '1.81', rmseV: '6.95',
        tval: '—', conclusion: 'Metode referensi',
        pipeline: 'SfM → COLMAP → MVS Dense',
        colors: [0x2DD4BF, 0x38BDF8, 0x4ADE80],
    },
    '3dgs': {
        label: '3DGS Lokal',
        desc: 'KSplat / PLY (koordinat lokal)',
        color: 0xF97316,
        ce90: '17.70', le90: '68.90', rmseH: '11.66', rmseV: '41.76',
        tval: '—', conclusion: 'Koordinat lokal — belum georeferensi',
        pipeline: 'SfM → Postshot 3DGS (lokal)',
        colors: [0xF97316, 0xFBBF24, 0xF87171],
    },
    geo3dgs: {
        label: 'Geo-3DGS',
        desc: 'KSplat + Transform XML (UTM 49S)',
        color: 0x818CF8,
        ce90: '17.70', le90: '68.90', rmseH: '11.66', rmseV: '41.76',
        tval: '-0.80', conclusion: 'H₀ tidak ditolak — setara SfM-MVS pada α=0.05',
        pipeline: 'SfM-GS + XML Transform T → UTM 49S',
        colors: [0x818CF8, 0x38BDF8, 0x2DD4BF],
    },
};

let currentMethod = 'sfmmvs';
let autoRotate = true;
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  body{font-family:'Space Grotesk',sans-serif;font-size:13px;background:#0D1117;color:#E2E8F0;overflow:hidden;height:100vh}
  ...
  .layer-cb{width:14px;height:14px;border:1px solid var(--border);border-radius:3px;display:flex;align-items:center;justify-content:center;transition:all .15s;flex-shrink:0}
  ...
  .hud-chip{background:rgba(13,17,23,.85);border:1px solid var(--border);border-radius:4px;padding:3px 8px;font-family:var(--mono);font-size:10px;color:var(--teal);backdrop-filter:blur(4px)}
  ...
  .sl{color:var(--muted)}.sv{color:var(--teal)}.sv.a{color:var(--amber)}
</style>
...
<div class="app">
  ...
  <div class="statusbar">
    ...
    <div class="sc"><span class="sl">RENDER</span><span class="sv">WebGL2 · Three.js</span></div>
...
let theta = Math.PI/4, phi = Math.PI/5, radius = 12;

const renderer = new THREE.WebGLRenderer({ canvas, antialias: true });
renderer.setPixelRatio(Math.min(devicePixelRatio, 2));

scene.add(new THREE.AmbientLight(0x223344, 3));
const sun = new THREE.DirectionalLight(0xB8D4FF, 2);
sun.position.set(10, 20, 10);
scene.add(sun);
scene.add(new THREE.DirectionalLight(0xFF8833, 0.4)).position.set(-8, 3, -8);

// Objects
const gridObj = new THREE.GridHelper(50, 50, 0x1E3A5F, 0x0F2235);
scene.add(gridObj);
scene.add(new THREE.AxesHelper(3));

let cloudObj = null;
const gcpGroup = new THREE.Group();
const icpGroup = new THREE.Group();
scene.add(gcpGroup);
scene.add(icpGroup);
...
[[-1.4,0,1.4],[1.4,0,1.4],[1.4,0,-1.4],[-1.4,0,-1.4]].forEach(p => {
    const m = new THREE.Mesh(new THREE.SphereGeometry(0.07,12,12),
        new THREE.MeshStandardMaterial({color:0xF97316,emissive:0xF97316,emissiveIntensity:.4}));
    m.position.set(...p);
    gcpGroup.add(m);
});
...
[[0,0,.9],[-.9,0,-.3],[.9,0,-.3]].forEach(p => {
    const m = new THREE.Mesh(new THREE.OctahedronGeometry(0.08),
        new THREE.MeshStandardMaterial({color:0xF87171,emissive:0xF87171,emissiveIntensity:.5}));
    m.position.set(...p);
    icpGroup.add(m);
});
...
function buildCloud(method) {
    ...
    for (let i=0;i<N;i++) {
        const a=Math.random()*Math.PI*2, h=rand(0,5);
        const r=rand(.1,.6)*(1-h*.08);
        pos[i*3]=Math.cos(a)*r; pos[i*3+1]=h; pos[i*3+2]=Math.sin(a)*r;
        const c=hexToRgb(d.colors[i%d.colors.length]);
        const br=rand(.5,1);
        col[i*3]=c[0]*br; col[i*3+1]=c[1]*br; col[i*3+2]=c[2]*br;
    }
    geo.setAttribute('position', new THREE.Float32BufferAttribute(pos,3));
    geo.setAttribute('color',    new THREE.Float32BufferAttribute(col,3));
    const mat = new THREE.PointsMaterial({size:.035,vertexColors:true,sizeAttenuation:true});
...
canvas.addEventListener('mousemove', e=>{
    if(!isDrag)return;
    theta-=(e.clientX-lastX)*.008;
    phi=Math.max(.1,Math.min(Math.PI/2.1,phi+(e.clientY-lastY)*.008));
    lastX=e.clientX;lastY=e.clientY;
    updateCam();
});
canvas.addEventListener('wheel', e=>{
    radius=Math.max(2,Math.min(50,radius+e.deltaY*.015));
...
(function loop(){
    ...
    const now=Date.now();
    if(now-lastT>800){
        document.getElementById('hFps').textContent=Math.round(fc*1000/(now-lastT))+' fps';
        fc=0;lastT=now;
    }
...
window.setMethod = function(method, el) {
    ...
    document.getElementById('sPipe').textContent   = d.pipeline;

    // Update metrics
    const ce90pct = method==='sfmmvs'?14:89;
    const le90pct = method==='sfmmvs'?16:99;
    const ce90color = method==='sfmmvs'?'#4ADE80':method==='geo3dgs'?'#F97316':'#F87171';
    const le90color = method==='sfmmvs'?'#F97316':'#F87171';

    document.getElementById('ce90val').innerHTML = d.ce90+' <span class="stat-unit">mm</span>';
    document.getElementById('ce90val').style.color = ce90color;
    document.getElementById('ce90bar').style.width = ce90pct+'%';
    document.getElementById('ce90bar').style.background = ce90color;
    document.getElementById('le90val').innerHTML = d.le90+' <span class="stat-unit">mm</span>';
    document.getElementById('le90val').style.color = le90color;
    document.getElementById('le90bar').style.width = le90pct+'%';
    document.getElementById('le90bar').style.background = le90color;
    ...
    document.getElementById('tconc').textContent = d.conclusion;
};
...
window.toggleRot = ()=>{
    ...
    document.getElementById('rotBtn').classList.toggle('on',autoRotate);
};
window.zoom = f=>{ radius=Math.max(2,Math.min(50,radius*f)); updateCam(); };